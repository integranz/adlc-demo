...
- Two layers, two lifecycles. `infra/foundation` (registry, key vault, identity, logs, role assignments) changes rarely and is applied by a human after `/adlc:plan dev --layer foundation` and an approval token. `infra/app` (compute + image tags) is applied only by `cd.yml`.
...
- Remote state lives in the storage account named in `.adlc/config.yaml` (`azure.state`); never commit `*.tfstate*` or `backend.hcl` with credentials.
