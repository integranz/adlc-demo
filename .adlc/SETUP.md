...
Generated by `/adlc:bootstrap` from `.adlc/config.yaml`. These are the one-time, human-only prerequisites; nothing here is created by an agent. Re-run the scaffold after changing options to refresh this file.

## 1. GitHub → `integranz/adlc-demo` → Settings → Secrets and variables → Actions
| Name | Kind | Value |
|---|---|---|
...
```
gh secret set AZURE_CLIENT_ID -R integranz/adlc-demo
gh secret set AZURE_TENANT_ID -R integranz/adlc-demo
gh secret set AZURE_SUBSCRIPTION_ID -R integranz/adlc-demo
gh secret set DOCKERHUB_TOKEN -R integranz/adlc-demo
gh variable set DOCKERHUB_USERNAME -R integranz/adlc-demo --body "<docker hub username>"
```
...
az ad app federated-credential create --id "$APP_OBJ" --parameters '{
  ...
  "subject": "repo:integranz/adlc-demo:ref:refs/heads/main",
  "audiences": ["api://AzureADTokenExchange"] }'
az ad app federated-credential create --id "$APP_OBJ" --parameters '{
  "name": "github-env-dev", "issuer": "https://token.actions.githubusercontent.com",
  "subject": "repo:integranz/adlc-demo:environment:dev",
  "audiences": ["api://AzureADTokenExchange"] }'
...
## 6. Local shell (for `/adlc:plan` and the human `terraform apply` of `infra/foundation`)
```
...
## 7. Check
```
gh secret list -R integranz/adlc-demo; gh variable list -R integranz/adlc-demo
gh api repos/integranz/adlc-demo/environments/dev --jq '.protection_rules[].type'
